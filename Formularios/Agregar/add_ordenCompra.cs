...
public partial class add_ordenCompra
{
    ...
    private void cmd_add_item_Click(object sender, EventArgs e)
    {
        ...
        {
            edita_item = mitem.info_item(srch.SelectedIndex);
    ...
    private void add_ordenCompra_Load(object sender, EventArgs e)
    {
        ...
        if (edicion | borrado)
        {
            ...
            lbl_fechaCarga.Text = ConversorFechas.GetFecha(edita_ordenCompra.FechaCarga);
            dtp_fechaComprobante.Value = ConversorFechas.GetFecha(edita_ordenCompra.FechaComprobante, dtp_fechaComprobante);



            cmb_proveedor.SelectedValue = edita_ordenCompra.IdProveedor;

            actualizarDataGrid();

            txt_subTotal.Text = edita_ordenCompra.Subtotal.ToString();
            txt_impuestos.Text = edita_ordenCompra.Iva.ToString();
            txt_total.Text = edita_ordenCompra.Total.ToString();
            txt_totalO.Text = txt_total.Text;
            txt_nota.Text = edita_ordenCompra.Notas;
            chk_secuencia.Enabled = false;

            lbl_nOrdenCompra.Text = edita_ordenCompra.IdOrdenCompra.ToString();
        ...
        if (edita_ordenCompra.IdOrdenCompra != 0 | borrado == true)
        {
        ...
        if (borrado == true)
        {
            ...
            {
                if (ordenesCompras.borrarOrdenCompra(edita_ordenCompra) == false)
                {
                    if (Interaction.MsgBox("Ocurrió un error al realizar el borrado de la orden de compra, ¿desea intetar desactivarla para que no aparezca en la búsqueda?", (MsgBoxStyle)((int)MsgBoxStyle.Question + (int)MsgBoxStyle.YesNo)) == Constants.vbYes)
                    {
                        ...
                        if (ordenesCompras.updateOrdenCompra(edita_ordenCompra, true) == true)
                        {
    ...
    private void cmd_ok_Click(object sender, EventArgs e)
    {
        ...
        if (edicion == true)
        {
            ...
            ordenesCompras.guardarOrdenCompra(edita_ordenCompra.IdOrdenCompra);
            Borrar_tabla_segun_usuario("tmpOC_items", usuario_logueado.IdUsuario);
        }
        else
        {
    ...
    private void dg_view_DoubleClick(object sender, EventArgs e)
    {
        ...
        edita_item = mitem.info_item((int)Conversion.Int(seleccionado));

        // Obtengo el ID interno de la tabla tmpOC_items
        seleccionado = dg_viewOC.CurrentRow.Cells[0].Value.ToString();
        edita_item.IdItemTemporal = (int)Conversion.Int(Strings.Left(seleccionado, Strings.InStr(seleccionado, "-") - 1));
    ...
    private void actualizarDataGrid()
    {
        ...
        if (edicion)
        {
            query = query.Where(t => t.IdOrdenCompra == edita_ordenCompra.IdOrdenCompra);
        }
        else
        {
    ...
    private void ModificarCantidadRecibidaToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        using (var context = new CentrexDbContext())
        {
            int idTmp = Convert.ToInt32(edita_item.IdItemTemporal);
