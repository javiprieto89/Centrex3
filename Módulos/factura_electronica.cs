...
// Propiedades públicas para acceso desde WSFEv1
 ...
 private static string password_certificado;
    ...
    private const string WSFE_PROD = "https://servicios1.afip.gov.ar/wsfev1/service.asmx";
        ...
        /// </summary>
        public static int facturar(pedido p)
        {
// SI DEVUELVE 1 ESTÁ TODO OK
  ...
  pedido pAnulado;
    comprobante cAnulado;
    string _fechaAFIP;

// Obtengo los datos del comprobante
    c = comprobantes.info_comprobante(p.IdComprobante.ToString());
  ...
  VariablesGlobales.UpdatePedido(p);
   return 1;
  }
    ...
    cl =clientes.info_cliente(p.IdCliente.ToString());
  _fechaAFIP = generales.fechaAFIP(); // Fecha de hoy

     // Inicializar configuración AFIP
     if (!inicialesFE(c.Testing))
          {
Interaction.MsgBox("Hubo un problema al inicializar el webservice, puede ser un problema de certificados", 
   ...
   return 0;
       ...
       // Crear conexión con WSFE usando la nueva infraestructura
          var afipMode = c.Testing ? AfipMode.HOMO : AfipMode.PROD;
          WSFEv1 wsfe = null;
   ...
   // Obtener último número de comprobante
        ...
        try
        {
         ultimoNro = wsfe.FECompUltimoAutorizado(c.PuntoVenta ?? 0, c.IdTipoComprobante ?? 0);
        }
      catch (Exception ex)
        {
 ...
 req.FeCabReq = new Centrex.Afip.Proxy.FECabReq()
        {
            CantReg = 1,
        CbteTipo = c.IdTipoComprobante ?? 0,
            PtoVta = c.PuntoVenta ?? 0
   };
...
// IVA
 ...
 opcionales.Add(new Centrex.Afip.Proxy.FEOpcional()
   ...
   Id = 2102,
       ...
       if ((c.IdModoMiPyme ?? 0) > 0)
   {
       var modoMiPyme = info_modoMiPyme(c.IdModoMiPyme ?? 0);
     if (modoMiPyme != null)
         {
     opcionales.Add(new Centrex.Afip.Proxy.FEOpcional()
   {
        ...
        if (c.AnulaMiPyME?.ToUpper() == "S" || c.AnulaMiPyME?.ToUpper() == "N")
 {
     ...
     det.Opcionales = opcionales.ToArray();
        ...
        // Documentos asociados para NC/ND MiPyME
         if (EsNotaCreditoODebito(c.IdTipoComprobante ?? 0) && (p.NumeroPedidoAnulado ?? 0) > 0)
             {
    pAnulado = VariablesGlobales.InfoPedido(p.NumeroPedidoAnulado?.ToString());
      if (pAnulado != null)
              {
        cAnulado = comprobantes.info_comprobante(pAnulado.IdComprobante?.ToString());
     if (cAnulado != null)
     {
 det.CbtesAsoc = new Centrex.Afip.Proxy.FECbtesAsoc[]
     {
       new Centrex.Afip.Proxy.FECbtesAsoc()
        {
     Tipo = cAnulado.IdTipoComprobante ?? 0,
   PtoVta = pAnulado.PuntoVenta ?? 0,
 Nro = p.NumeroComprobanteAnulado ?? 0,
       Cuit = Conversions.ToLong(VariablesGlobales.cuit_emisor_default?.Replace("-", "") ?? "0"),
       CbteFch = _fechaAFIP
   }
    };
        }
...
p.CodigoDeBarras = generarCodigoDeBarras(VariablesGlobales.cuit_emisor_default, p.NumeroComprobante ?? 0, p.PuntoVenta ?? 0, p.Cae, detResp.CAEFchVto);
        ...
        if (GenerarYGuardarQR_AFIP(c, p, cl, nombreQR, afipMode))
 {
  ...
  try
            {
           string pc = VariablesGlobales.pc;
      ...
      else
{
 ...
 return "";
        ...
        /// </summary>
        private static bool GenerarYGuardarQR_AFIP(comprobante c, pedido p, cliente cl, string rutaArchivo, AfipMode modo)
        {
     try
   {
   ...
   try
{
  ...
  return "Error al procesar mensaje";
    ...
    /// </summary>
        private static string errorFE_new(WSFEv1 wsfe, comprobante c, int ultimoNro)
        {
            string errorStr = "";
  ...
  /// Imprime factura - MANTIENE LA FIRMA ORIGINAL
        ...
        {
  if (VariablesGlobales.showrpt)
   {
 VariablesGlobales.id = id_pedido;
      ...
      frm.ShowDialog();
        ...
        /// <summary>
      /// Consulta último comprobante - MANTIENE LA FIRMA ORIGINAL
        /// </summary>
        public static int consultaUltimoComprobante(string puntoVenta, string tipoComprobante, bool esTest)
        {
      try
            {
      // Inicializar configuración AFIP
    ...
    return -1;
              ...
              // Generar login CMS primero
      if (!GenerarLoginCms(esTest))
                {
          Interaction.MsgBox("Error al obtener el ticket de acceso, PROBLEMA DE AFIP", 
            (MsgBoxStyle)((int)Constants.vbCritical + (int)Constants.vbOKOnly), "Centrex");
        return -1;
      }

    // Crear instancia WSFEv1 con token y cuit
 var wsfe = WSFEv1.CreateWithTa(token, cuit_emisor);
      
      // Consultar último comprobante autorizado
    int ultimoCmp = wsfe.FECompUltimoAutorizado(Conversions.ToInteger(puntoVenta), Conversions.ToInteger(tipoComprobante));
                
  WriteLog("consultaUltimoComprobante", $"Último comprobante: PV={puntoVenta}, Tipo={tipoComprobante}, Nro={ultimoCmp}");
                return ultimoCmp;
            }
            catch (Exception ex)
   {
    WriteLog("consultaUltimoComprobante", $"Error: {ex.Message}");
    Interaction.MsgBox($"Error al consultar último comprobante: {ex.Message}", 
  (MsgBoxStyle)((int)Constants.vbCritical + (int)Constants.vbOKOnly), "Centrex");
       return -1;
         }
    ...
    // Esta función debe ser implementada según la lógica de negocio
        ...
        /// <summary>
        /// Inicializa configuración AFIP - MÉTODO PÚBLICO PARA COMPATIBILIDAD
  /// </summary>
        public static bool InicialesFE(bool esTest)
        {
            return inicialesFE(esTest);
        }
  ...
  /// </summary>
        public static object ConsultarComprobante(object wsfe, int puntoVenta, int tipoComprobante, long numeroComprobante)
     {
         try
    {
       if (wsfe != null)
